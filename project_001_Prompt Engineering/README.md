# Prompt Engineering

> Prompt Engineering 学习笔记索引，适合按“基础概念 -> 提示技术 -> 技术对比 -> 最佳实践”的顺序阅读。

- [00_Prompt Engineering 技术关系总览](<notes/00_Prompt Engineering技术关系总览.md>)

## Prompt Engineering 提示词工程基础概念

- [01_大预言模型设置](<notes/01_大预言模型设置.md>)
- [02_提示词构成要素](<notes/02_提示词构成要素.md>)
- [03_设计提示词的通用技巧](<notes/03_设计提示词的通用技巧.md>)

## 提示技术

- [04_零样本提示 Zero-shot](<notes/04_零样本提示 Zero-shot.md>)
- [05_少样本提示 Few-shot](<notes/05_少样本提示 Few-shot.md>)
- [06_链式思考（CoT）提示](<notes/06_链式思考（CoT）提示.md>)
- [07_思维树 ToT](<notes/07_思维树 ToT.md>)
- [08_自我一致性 Self-Consistency](<notes/08_自我一致性 Self-Consistency.md>)
- [09_链式提示 Prompt Chaining](<notes/09_链式提示 Prompt Chaining.md>)
- [10_ReAct 框架](<notes/10_ReAct 框架.md>)
- [11_自我反思 Reflexion](<notes/11_自我反思 Reflexion.md>)

## 技术关联/区别/对比

- [00_Prompt Engineering 技术关系总览](<notes/00_Prompt Engineering技术关系总览.md>)
- [12_零样本提示和少样本提示的关系](<notes/12_零样本提示和少样本提示的关系.md>)
- [13_CoT 和 Prompt Chaining 的区别](<notes/13_CoT 和 Prompt Chaining的区别.md>)
- [14_ReAct 和 Reflexion 的区别](<notes/14_ReAct 和 Reflexion的区别.md>)
- [15_CoT 和 ToT 的区别](<notes/15_CoT 和 ToT的区别.md>)
- [16_CoT 和自我一致性的区别](<notes/16_CoT 和自我一致性的区别.md>)
- [17_Prompt Chaining 和 ReAct 的区别](<notes/17_Prompt Chaining 和 ReAct的区别.md>)
- [18_自我一致性和 Reflexion 的区别](<notes/18_自我一致性和 Reflexion的区别.md>)

## 最佳实践

- [26条有效提示词技巧](<notes/26条有效提示词技巧.md>)
- [结构化 Prompt 编写指南和最佳实践](<notes/结构化 Prompt 编写指南和最佳实践.md>)

## 编写 Prompt Skill

- [Agent Prompt Writer](https://github.com/xinghe-AGI/Agent-Prompt-Writer)

## 项目实践

- [Prompt Template 学习项目](<code/02_Study_PromptTemplate/README.md>)：学习系统 Prompt、用户 Prompt、模板变量、动态传参以及 Agent 加载 Prompt 的完整过程。

## 参考资料

- [提示工程指南](https://www.promptingguide.ai/zh)
- [NanGePlus/PromptsCollection](https://github.com/NanGePlus/PromptsCollection)
- [OpenAI Prompt engineering](https://developers.openai.com/api/docs/guides/prompt-engineering)
- [Anthropic Prompt Engineering Interactive Tutorial](https://github.com/anthropics/prompt-eng-interactive-tutorial)
