...
| 组成部分 | 使用方式 | 主要作用 | 写作重点 |
|---|---|---|---|
| 任务 | 通用必需 | 说明模型要完成哪些动作 | 写清目标、动作和任务边界，不写空泛口号 |
| 输入 | 通用必需 | 定义运行时材料及其边界 | 使用占位符或分隔符标明输入，不让规则和数据混在一起 |
| 约束条件 | 通用必需 | 写明必须遵守的规则 | 包括允许范围、禁止项、异常处理和安全边界 |
| 输出 | 通用必需 | 定义最终交付物 | 写清形态、字段、类型、枚举和顺序 |
| 完成标准 | 通用必需 | 说明什么结果才算完成 | 使用可以检查的条件，不写“高质量”等模糊要求 |
| 角色 | 条件模块 | 收窄任务域和责任范围 | 只有角色会实际改变行为时才加入 |
| 背景 | 条件模块 | 补充输入来源、使用场景和易错点 | 只保留影响判断的上下文 |
| 工作流 | 复杂任务按需 | 把任务拆成可执行步骤 | 步骤有依赖、需要检查或调用工具时再加入 |
| 示例 | 边界不清时按需 | 对齐判断标准和输出模式 | 覆盖代表性情况，必要时加入异常样例 |
| 版本信息 | 长期维护时按需 | 记录作者、版本和变更内容 | Prompt 进入团队或自动化流程后再维护 |
| 初始化 | 长期角色或 Agent 按需 | 说明首轮如何开始 | 简短提示用户提供必要输入，不重复整份规则 |

**我的理解：这套骨架像是在给模型写一份岗位说明 + 操作手册 + 接口契约。**

普通 prompt 只告诉模型“帮我做什么”，结构化 prompt 还会告诉模型“输入是什么、规则是什么、结果必须长什么样、异常怎么处理”。

不过，结构化不等于把所有模块全部写一遍。**任务、输入、约束、输出和完成标准负责形成最小闭环；其他模块只有在确实会改变模型行为、输出或边界时才加入。** 简单任务可以很短，复杂任务才需要角色、背景、工作流、示例、版本和初始化。
...
下面的完整模板适合字段较多、需要长期维护或接入工作流的 Prompt。简单任务应先保留通用必需模块，再按实际需要增加其他部分，不要机械套满所有标题。
