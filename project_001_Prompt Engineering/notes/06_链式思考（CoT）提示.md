...
这和 [[05_少样本提示 Few-shot]] 的关系很近。普通 few-shot 示例通常只是“输入 -> 输出”，而少样本 CoT 示例是“输入 -> 推理链 -> 输出”。
...
- **可以和自洽性结合**， 对复杂问题可以多次生成不同推理链，再用 [[08_自我一致性 Self-Consistency]] 的方式选择更稳定的最终答案。
...
- [[15_CoT 和 ToT的区别]]：区分线性推理和多分支搜索。
- [[13_CoT 和 Prompt Chaining的区别]]：区分单次回答内部推理链和多次调用之间的任务链。
- [[00_Prompt Engineering技术关系总览]]：把 CoT 放回推理增强层中，和 ToT、自我一致性一起比较。
