...
**自我一致性（Self-Consistency）是一种通过多次采样推理路径，并从多个最终答案中选择最一致答案的推理增强方法。** 它通常和 [[06_链式思考（CoT）提示]] 一起使用：CoT 让模型写出一条推理链，自我一致性则让模型写出多条可能的推理链，再看最终答案是否收敛。
...
自我一致性通常建立在 CoT 之上，但它和 CoT 解决的问题不同。

| 维度 | CoT | Self-Consistency |
|---|---|---|
| 核心动作 | 生成一条推理链 | 生成多条推理链并聚合答案 |
| 主要目标 | 让模型展开中间步骤 | 降低单条路径偶然错误 |
| 成本 | 增加一次推理长度 | 增加多次推理调用 |
| 适合问题 | 需要分步推理的问题 | 分步推理后仍然不稳定的问题 |
| 输出重点 | 推理过程和答案 | 答案分布和最终选择 |

**可以把 CoT 理解为“让模型认真做一遍”，把自我一致性理解为“让模型从不同路线多做几遍，再看答案是否一致”。** 所以它不是 CoT 的替代品，而是 CoT 不够稳定时的增强层。

![CoT 和自我一致性怎么配合](picture/03-cot-self-consistency-decision.png)
...
- [[00_Prompt Engineering技术关系总览]]：把自我一致性和 CoT、ToT、Reflexion 放在一起比较，理解“多次采样投票”和“反馈后改进”的区别。
