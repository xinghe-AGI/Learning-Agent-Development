...
**思维树（Tree of Thoughts, ToT）是一种让大模型同时探索多条推理路径，并对中间思路进行评估、选择、回溯的提示和推理框架。** 如果说 [[06_链式思考（CoT）提示]] 是把一条推理链写出来，那么 ToT 更像是把多个可能的推理方向展开成一棵树，再从里面找出更有希望的路径。
...
- **可以和其他方法组合**， ToT 可以结合 [[08_自我一致性 Self-Consistency]]、工具调用、规则检查器或搜索算法，让评估环节更可靠。
...
ToT 可以看作 CoT 的扩展，但它不是简单地“写更长的思维链”。

| 维度 | CoT | ToT |
|---|---|---|
| 推理形态 | 一条线性推理链 | 多条分支推理树 |
| 关键动作 | 分步推理 | 生成候选、评估、搜索、回溯 |
| 适合问题 | 多步但路径相对明确 | 路径不确定、需要试错和选择 |
| 成本 | 中等 | 更高 |
| 风险 | 早期步骤错了会一路错下去 | 搜索和评估设计不好会浪费大量成本 |

**一个实用判断是：能沿一条清晰路线解决的问题，先用 CoT；需要同时比较多条路线的问题，再考虑 ToT。**

![CoT 和 ToT 怎么选](picture/03-cot-vs-tot-comparison.png)
...
- [[15_CoT 和 ToT的区别]]：专门比较一条推理链和多分支搜索树的差异。
- [[00_Prompt Engineering技术关系总览]]：把 ToT 和 CoT、自我一致性放在同一层看，理解它适合什么时候升级使用。
