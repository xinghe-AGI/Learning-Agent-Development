...
当前项目在 `code/utils/models.py` 中使用 **`@dataclass`** 定义结构化响应：
...
1. `code/utils/models.py` 定义 `WeatherResponseFormat`，约定最终结果的字段和类型。
2. `code/agent.py` 把它传给 `create_agent(response_format=...)`。
