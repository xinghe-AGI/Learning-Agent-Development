...
| `build_response_record(response)` | `code/utils/call_records.py` | 把 Agent state 整理成便于记录的调用摘要 |
...
项目中的 `code/agent.py` 会在本地运行时开启 tracing，并从外部环境读取 API Key：
...
`code/utils/call_records.py` 的入口函数是：
