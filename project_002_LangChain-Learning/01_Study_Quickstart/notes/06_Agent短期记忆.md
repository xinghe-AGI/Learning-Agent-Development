...
[agent.py](<../code/agent.py>) 中先创建 checkpointer：
...
完整消息结构可以参考 [07_模型请求与响应结构](<07_模型请求与响应结构.md>)，工具调用消息可以参考 [04_Tools与FunctionCalling](<04_Tools与FunctionCalling.md>)。
...
结构化输出细节可以参考 [03_结构化输出](<03_结构化输出.md>)，Schema 建模可以参考 [18_Pydantic增强Schema与dataclass区别](<18_Pydantic增强Schema与dataclass区别.md>)。
...
- [05_create_agent参数详解](<05_create_agent参数详解.md>)：理解 `checkpointer`、`context_schema`、`state_schema` 和 `store` 的位置。
- [07_模型请求与响应结构](<07_模型请求与响应结构.md>)：观察 `messages` 中的用户消息、模型消息和工具消息。
- [04_Tools与FunctionCalling](<04_Tools与FunctionCalling.md>)：理解工具调用结果怎样写入线程消息状态。
- [03_结构化输出](<03_结构化输出.md>)：理解 `structured_response` 与 Agent state 的关系。
- [18_Pydantic增强Schema与dataclass区别](<18_Pydantic增强Schema与dataclass区别.md>)：理解结构化对象进入状态后的建模选择。
- [08_LangSmith跟踪与调用记录](<08_LangSmith跟踪与调用记录.md>)：观察多轮调用、Token 累计和线程级消息轨迹。
