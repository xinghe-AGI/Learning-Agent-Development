...
这个项目把模型接入集中到了 `code/utils/llms.py`。`code/agent.py` 不需要分别了解 OpenAI、Qwen、OneAPI 或 Ollama 的环境变量和初始化参数，只需要把 `Config.LLM_TYPE` 交给 `get_llm()`，再接收统一返回的 Chat Model 与 Embedding 对象。
...
| `get_llm()` | 给 `code/agent.py` 提供统一入口，并在初始化失败时执行默认模型回退 |
...
`code/agent.py` 因此只保留一行模型接入代码：

```python
llm_chat, llm_embedding = get_llm(Config.LLM_TYPE)
```

后面创建 Agent 时只传入 `llm_chat`。`code/agent.py` 不需要知道当前对象来自 OpenAI、Qwen、OneAPI 还是 Ollama，也不需要重复模型初始化参数。
