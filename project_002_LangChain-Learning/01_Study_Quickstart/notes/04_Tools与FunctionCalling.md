...
| 概念 | 关注点 | 当前项目中的例子 |
| --- | --- | --- |
| Tool 定义 | 给模型看见哪些工具、参数和描述 | `code/utils/tools.py` 中的 `@tool(...)` |
...
| 3. 执行工具 | LangChain Agent runtime | 调用 `code/utils/tools.py` 中的 Python 函数 |
...
当前项目的工具集中在 [tools.py](<../code/utils/tools.py>)，由 `get_tools()` 返回：
...
然后在 [agent.py](<../code/agent.py>) 中交给 Agent：
