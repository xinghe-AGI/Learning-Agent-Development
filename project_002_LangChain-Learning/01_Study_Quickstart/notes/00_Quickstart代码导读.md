...
| 顺序 | 文件 | 这一遍重点看什么 |
| --- | --- | --- |
| 1 | `code/agent.py` | Agent 怎样创建、怎样调用，以及当前多轮实验在验证什么 |
| 2 | `code/utils/models.py` | `Context` 和 `WeatherResponseFormat` 分别描述哪类数据 |
| 3 | `code/utils/tools.py` | `@tool` 怎样暴露工具，`ToolRuntime` 怎样读取上下文 |
| 4 | `code/utils/llms.py` | `Config.LLM_TYPE` 怎样变成 Chat Model 和 Embedding 对象 |
| 5 | `code/utils/call_records.py` | 怎样从返回消息中提取模型、Token、结束原因和工具调用 |
| 6 | `code/utils/config.py`、`code/utils/logger.py` | 模型选择、日志路径和日志轮转怎样配置 |

## 从 `code/agent.py` 找到主线

### 创建 Agent 之前准备了什么

`code/agent.py` 在创建 Agent 前依次准备四类对象：
...
`code/agent.py` 当前实际执行三个 `invoke()`：线程 1 连续调用两次，再使用全新的线程 3 调用一次；线程 2 的上海示例处于注释状态。
...
`code/utils/models.py` 定义了两种用途完全不同的数据模型：
...
`code/agent.py` 通过下面的代码把 Agent 返回状态整理为调用记录：
...
- **LangSmith 配置依赖外部环境变量**：`code/agent.py` 会开启 tracing，并从外部环境读取 `LANGSMITH_API_KEY` 或兼容旧变量名 `LANGCHAIN_API_KEY`；密钥不要写入代码或笔记。
- **脚本没有主入口保护**：`code/agent.py` 没有 `if __name__ == "__main__":`，因此导入模块也会初始化模型并执行未注释的三次 `invoke()`。
