...
这种方式适合多厂商或 OpenAI-compatible 接入，因为模型初始化细节集中在 `utils/llms.py`。`create_agent()` 只接收已经配置好的 `llm_chat`，不再关心底层 `base_url` 和 API Key。
...
项目中工具由 [tools.py](<../utils/tools.py>) 提供：
