...
当前 `utils/llms.py` 真正生效的是直接创建 `ChatOpenAI`。`init_chat_model()` 只作为需要理解的另一种模型工厂写法，不属于当前 Agent 的实际运行路径。
...
当前 `utils/llms.py` 没有使用 `_ConfigurableModel`。项目先通过 `Config.LLM_TYPE` 选择一组配置，再直接创建固定的 `ChatOpenAI` 对象。
