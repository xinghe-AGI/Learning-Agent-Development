...
项目入口在 [agent.py](<../agent.py>)。一次 Agent 调用分为两个阶段：先装配 Agent，再执行 Agent。
...
工具调用的细节可以参考 [[04_Tools与FunctionCalling]]。
...
[call_records.py](<../utils/call_records.py>) 的作用是把 Agent state 整理成便于记录的摘要。
...
- [[01_多厂商LLM集成与API协议]]：理解 OpenAI-compatible 接入和 Provider 差异。
- [[10_ChatOpenAI对象详解]]：理解模型初始化参数、调用参数和返回对象。
- [[02_模型采样参数与ChatOpenAI]]：理解 temperature、top_p 等字段怎样影响生成。
- [[04_Tools与FunctionCalling]]：理解工具定义、工具调用意图和结果回传。
- [[03_结构化输出]]：理解 `response_format`、ToolStrategy 与 ProviderStrategy。
- [[05_create_agent参数详解]]：理解 Agent 装配参数与 runtime 输入。
- [[06_Agent短期记忆]]：理解 `thread_id`、checkpointer 和 messages 状态。
- [[08_LangSmith跟踪与调用记录]]：理解如何观察完整调用链、Token、延迟和错误。
