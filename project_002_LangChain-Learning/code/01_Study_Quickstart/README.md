...
| 学习主题 | 重点理解的问题 | 对应代码 |
| --- | --- | --- |
| 各厂商 LLM 集成 | 原生 Provider 与 OpenAI-compatible 接入有什么区别？ | `utils/llms.py` |
| 模型采样参数 | `temperature`、`top_p`、`top_k`、penalty 和输出长度怎样影响生成？ | `utils/llms.py` |
| 结构化输出 | JSON Mode、JSON Schema、Pydantic Schema、ToolStrategy、ProviderStrategy 有什么区别？ | `utils/models.py`、`agent.py` |
| Tools 与 Function Calling | 模型怎样提出调用意图，Agent runtime 怎样执行并回传结果？ | `utils/tools.py`、`agent.py` |
| `create_agent()` | 可以传入哪些对象？state、context 与 config 有什么区别？ | `agent.py` |
| Agent 短期记忆 | checkpointer 和 `thread_id` 怎样共同维护、隔离历史？ | `agent.py` |
| 请求与响应结构 | `model`、`messages/input`、instructions、tools、stream、usage 等字段位于哪一层？ | `agent.py`、`utils/call_records.py` |
| API 协议模型 | Chat Completions、Responses、Anthropic Messages 怎样表达消息和工具结果？ | `agent.py`、LangChain message 对象 |
| LangSmith 与调用记录 | 怎样观察模型调用、工具步骤、Token、延迟和错误？ | `agent.py`、`utils/call_records.py` |
...
├── tests/
│   └── test_call_records.py              # 调用记录提取逻辑的单元测试
└── logfile/                              # 运行日志目录
```

配图统一存放在上级学习项目的 [notes/notes_pictures](<../../notes/notes_pictures>) 目录中，笔记通过相对路径引用。
...
1. 参考 [.env.example](<.env.example>) 创建本机 `.env`。
2. 填写准备使用的模型服务 URL、API Key、Chat Model 和 Embedding Model。
3. 在 `utils/config.py` 中通过 `Config.LLM_TYPE` 选择 `openai`、`qwen`、`oneapi` 或 `ollama`。
