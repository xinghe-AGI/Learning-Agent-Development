...
'''
logger.py  定义日志管理器类,用于集中管理日志配置和提供统一的获取接口
包含
def __new__(cls)            使用单例模式创建一个LoggerManager
def __init__(self)          首次初始化时配置日志记录器
def _setup_logger(self)     配置日志记录器基本配置
def logger(self)            返回内部持有的 logging.Logger 对象
def get_logger(cls)         通过类本身创建/获取单例 LoggerManager 实例
'''
