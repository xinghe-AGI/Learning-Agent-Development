...
'''
    config.py   定义一个统一配置类，用于集中管理项目中的所有常量配置
    包含
    LOG_FILE        配置日志文件路径，用于持久化存储应用运行日志
    MAX_BYTES       配置单个日志文件的最大字节数（这里是 5MB），通常用于配合轮转日志处理
    BACKUP_COUNT    配置日志轮转时最多保留的备份文件数量，这里设置为保留 3 个历史日志文件
    LLM_TYPE        配置使用的大模型类型
'''
